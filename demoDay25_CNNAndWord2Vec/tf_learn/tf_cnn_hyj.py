...
# 定义网络层的权重和偏置。
#注意： 每层的输出channel可以自定义，[5, 5, 1, 32]表示5*5的卷积核，1个输入通道channel，32个输出channel
# 第一个 conv 层有一个 5×5 的卷积核，1 个输入和 32 个feature map输出（32可自定义）。
# 第二个 conv 层有一个 5×5 的卷积核，32 个输入和 64 个输出（64可自定义）。
